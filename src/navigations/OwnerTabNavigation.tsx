...
export type OwnerTabParamList = {
    dashboard: undefined;
    bookings: undefined;
    addVenue: undefined;   // center FAB — navigates to an Add Venue stack screen
    messages: undefined;
    profile: undefined;
};
...
const TABS = [
    ...
    { name: "addVenue", label: "Add Venue", icon: "add-circle", iconOff: "add-circle-outline", center: true },
...
function CenterTab({ isFocused, onPress, tabWidth }: { isFocused: boolean; onPress: () => void; tabWidth: number }) {
    ...
    useEffect(() => {
        Animated.spring(scale, { toValue: isFocused ? 1.12 : 1, useNativeDriver: true, speed: 24, bounciness: 12 }).start();
    }, [isFocused]);

    const handlePress = () => {
        Animated.sequence([
            Animated.timing(scale, { toValue: 0.88, duration: 80, useNativeDriver: true }),
            Animated.spring(scale, { toValue: isFocused ? 1.12 : 1, useNativeDriver: true, speed: 20, bounciness: 14 }),
        ]).start();
        onPress();
    };
...
function AddVenuePlaceholder() {
    return (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.background }}>
            <Ionicons name="business-outline" size={48} color={Colors.primaryBorder} />
            <Text style={{ fontSize: 18, fontWeight: '800', color: Colors.charcoal, marginTop: 12 }}>Add New Venue</Text>
            <Text style={{ fontSize: 14, color: Colors.charcoalLight, marginTop: 6 }}>Build your add venue form here</Text>
        </View>
    );
}
...
export function OwnerTabNavigation() {
    return (
        <Tabs.Navigator tabBar={(props) => <OwnerTabBar {...props} />} screenOptions={{ headerShown: false }}>
            <Tabs.Screen name="dashboard" component={OwnerDashboardScreen} />
            <Tabs.Screen name="bookings" component={BookingsScreen} />
            <Tabs.Screen name="addVenue" component={AddVenuePlaceholder} />
