...
export default function BrowseCategoryScreen({ navigation }: categoryProps) {
    // FIX: typed state instead of untyped []
    ...
    // FIX: call fetchCategory on mount via useEffect
    useEffect(() => {
        fetchCategory();
    }, []);
    ...
    // FIX: filter/sort on `categories` state — not the undefined `data` variable
    ...
    const renderCard = useCallback(
        ({ item }: { item: VenueType; index: number }) => {
            return (
                <TouchableOpacity
                    style={s.card}
                    activeOpacity={0.82}
